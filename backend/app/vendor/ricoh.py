...
class RicohVendor(BasePrinterVendor):
    def detect(self, sys_descr: str, sys_object_id: str) -> bool:
        return "ricoh" in sys_descr.lower() or "1.3.6.1.4.1.367" in sys_object_id

    def read_counter(self) -> Optional[int]:
        # Walk index table
        index_tree = self.snmp_walk(self.ip, "1.3.6.1.4.1.367.3.2.1.2.19.5.1.5")
        if index_tree:
            for oid, val in index_tree.items():
                if val and "Machine Total" in val:
                    idx = oid.split(".")[-1]
                    val_oid = f"1.3.6.1.4.1.367.3.2.1.2.19.5.1.9.{idx}"
