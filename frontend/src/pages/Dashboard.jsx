...
const TonerMini = ({ level, color }) => {
  ...
  const isLow = pct < 20;
  return (
    <span style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', gap: 2, width: 28 }}>
      <span style={{ fontSize: '0.6rem', color: isLow ? '#F87171' : 'var(--text-muted)', fontWeight: 600, width: '100%', textAlign: 'center' }}>{pct}%</span>
      <span style={{
...
const Dashboard = () => {
  const [summary, setSummary] = useState({
    total: 0, apeos4620: 0,
    low_toner: 0, fuji_printers: [], other_printers: [], low_toner_printers: [], error_printers: []
  });
  ...
  const statCards = [
    ...
    { label: 'Active Errors', value: summary.error_printers?.length || 0, icon: <AlertTriangle size={22} />, color: '#EF4444' },
  ];
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginTop: '1rem' }}>
            ...
            <div className="glass-card">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
                ...
                <h3 style={{ fontWeight: 600, fontSize: '1rem' }}>Fuji Apeos Printers</h3>
              </div>

              {!summary.fuji_printers || summary.fuji_printers.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '1.5rem 0' }}>No Fuji Apeos printers online.</p>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                      {['IP Address', 'Model', 'Pages', 'Drum', 'Toner'].map(h => (
                  ...
                  <tbody>
                    {summary.fuji_printers.map(p => (
                      <tr key={p.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        <td style={{ padding: '0.6rem 0.75rem', fontWeight: 500, fontSize: '0.85rem' }}>
                          <a href={`http://${p.ip_address}`} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--info)', textDecoration: 'none' }}>{p.ip_address}</a>
                        </td>
                        ...
                        <td style={{ padding: '0.6rem 0.75rem' }}>
                          {p.drum_unit !== null && p.drum_unit !== undefined ? (
                            <span style={{ display: 'inline-block', width: '40px', fontSize: '0.85rem', fontWeight: 600, color: p.drum_unit < 20 ? '#F87171' : 'var(--success)' }}>
                              {p.drum_unit}%
                            </span>
                          ) : (
            ...
            <div className="glass-card">
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
                ...
                <h3 style={{ fontWeight: 600, fontSize: '1rem' }}>Other Printers</h3>
              </div>

              {!summary.other_printers || summary.other_printers.length === 0 ? (
                <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '1.5rem 0' }}>No other printers online.</p>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr>
                    ...
                    return (
                      <div key={p.id} style={{ background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: 10, padding: '0.75rem 1rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                          <a href={`http://${p.ip_address}`} target="_blank" rel="noopener noreferrer" style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-main)', textDecoration: 'none' }}>{p.ip_address}</a>
            ...
            <div className="glass-card" style={{ gridColumn: '1 / -1' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1.25rem' }}>
                ...
                <h3 style={{ fontWeight: 600, fontSize: '1rem' }}>Active Errors & Jams</h3>
              </div>

              {!summary.error_printers || summary.error_printers.length === 0 ? (
                <p style={{ color: 'var(--success)', textAlign: 'center', padding: '1.5rem 0', fontWeight: 500 }}>✓ No active errors or jams</p>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                  {summary.error_printers.map(p => (
                    <div key={p.id} style={{ background: 'rgba(239,68,68,0.07)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: 10, padding: '0.75rem 1rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                        <a href={`http://${p.ip_address}`} target="_blank" rel="noopener noreferrer" style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--text-main)', textDecoration: 'none' }}>{p.ip_address}</a>
                        <span style={{ color: 'var(--text-muted)', fontSize: '0.8rem' }}>{p.model}</span>
                      ...
                      <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
                        {p.last_error && (
                          <span style={{ background: 'rgba(239,68,68,0.15)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 6, padding: '0.2rem 0.5rem', fontSize: '0.75rem', fontWeight: 600, color: '#F87171' }}>
                            Error: {p.last_error}
                          </span>
                        )}
                        {p.paper_jam_count > 0 && (
                          <span style={{ background: 'rgba(245,158,11,0.15)', border: '1px solid rgba(245,158,11,0.3)', borderRadius: 6, padding: '0.2rem 0.5rem', fontSize: '0.75rem', fontWeight: 600, color: '#F59E0B' }}>
                            Paper Jams: {p.paper_jam_count}
                          </span>
                        )}
                      </div>
                    </div>
